...
import Banner from '../assets/badge-prepation.png';

function PalaceCheckIn() {
  const [guestList, setGuestList] = useState([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [isCheckedInPointTwo, setIsCheckedInPointTwo] = useState(false);

  const fetchGuestData = () => {
    // Fetch the guest list from your backend
    axios
      .get('http://46.19.74.196:5000/api/guests/attended/absent')
      .then((response) => {
  ...
  const markAsAttended = (guestId) => {
    axios
      .put(`http://46.19.74.196:5000/api/guests/attended/${guestId}`, {
        action: 'attended',
      })
      .then((response) => {
  ...
  const handleCheckIn = (guestId) => {
    ...
    axios
      .put(`http://46.19.74.196:5000/api/guests/attended${guestId}`, {
        action: 'attended-point-two',
      })
      .then((response) => {
        setIsCheckedInPointTwo(true);
      })
      .catch((error) => {
